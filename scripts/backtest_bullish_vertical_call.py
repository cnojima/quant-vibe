"""Backtest the Bullish Vertical Call strategy with SPXW options data."""
...
def main():
    """Run backtest for Bullish Vertical Call strategy."""

    print("="*70)
    print("BULLISH VERTICAL CALL STRATEGY - BACKTEST")
    print("="*70)
    print()
    ...
    end_date = datetime(2025, 12, 12, 23, 59, 59)  # Include full day of Dec 12
    ...
    # Save trades
    if not results['trades'].empty:
        trades_file = output_dir / f"bullish_vertical_call_trades_{timestamp}.csv"
        results['trades'].to_csv(trades_file, index=False)
        print(f"\n✅ Trades saved to: {trades_file}")

    # Save equity curve
    if not results['equity_curve'].empty:
        equity_file = output_dir / f"bullish_vertical_call_equity_{timestamp}.csv"
