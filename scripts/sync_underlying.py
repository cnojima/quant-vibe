#!/usr/bin/env python3
"""Sync underlying_bars data from remote TimescaleDB to local instance."""
...
def get_last_sync_time(local_conn, ticker='SPX'):
...
def sync_underlying_bars_with_until(since_time=None, until_time=None, ticker='SPX', batch_size=10000):
    """Sync underlying_bars from remote to local with date range.
    ...
    Args:
        ...
        ticker: Ticker symbol to sync (default: SPX)
    ...
    print(f"{'='*70}")
    print(f"SYNCING UNDERLYING DATA FROM REMOTE")
    print(f"{'='*70}")
...
def sync_underlying_bars(since_time=None, ticker='SPX', batch_size=10000):
    """Sync underlying_bars from remote to local (backward compatible).
    ...
    Args:
        ...
        batch_size: Number of rows per batch insert
    """
    return sync_underlying_bars_with_until(since_time=since_time, until_time=None, ticker=ticker, batch_size=batch_size)


if __name__ == '__main__':
    import argparse

    parser = argparse.ArgumentParser(description='Sync underlying data from remote to local DB')
    ...
    parser.add_argument('--ticker', type=str, default='SPX', help='Ticker symbol (default: SPX)')
    ...
    if args.auto:
        ...
        local_conn = psycopg2.connect(**LOCAL_CONFIG)
        since_time = get_last_sync_time(local_conn, ticker=args.ticker)
        local_conn.close()
    ...
    if until_time and (until_time - since_time).days > args.batch_days:
        ...
        while current < until_time:
            ...
            sync_underlying_bars_with_until(since_time=current, until_time=batch_end, ticker=args.ticker)
        ...
        # Single sync
        if until_time:
            sync_underlying_bars_with_until(since_time=since_time, until_time=until_time, ticker=args.ticker)
        else:
            sync_underlying_bars(since_time=since_time, ticker=args.ticker)
