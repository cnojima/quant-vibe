...
## 🚨 CRITICAL - Do Before Live Trading (Week 1-2)

### [ ] 1. Enable Paper Trading Validation
**Priority**: CRITICAL  
**Estimated Time**: 1 hour setup + 2 weeks monitoring  
**Files**: `config/live_trading.yaml`

- [ ] Verify `paper_trading: true` in config
- [ ] Enable ONLY `bullish_vertical_put` strategy initially
- [ ] Set `position_size: 1` for conservative testing
- [ ] Run paper trading Mon-Fri for 2 weeks minimum
- [ ] Monitor via Admin UI: http://localhost:5173/live
- [ ] Track metrics daily:
  - [ ] Actual fills vs expected (slippage analysis)
  - [ ] Entry timing accuracy
  - [ ] Exit execution quality
  - [ ] P&L vs backtest predictions
- [ ] Document any discrepancies in `reports/paper_trading_analysis.md`

**Success Criteria**: <5% difference between backtest and paper trading results
...
**Files**: `src/quant_vibe/notifications/pushover.py`, `.env`

#### Setup
- [ ] Sign up for Pushover account: https://pushover.net/
- [ ] Purchase Pushover app ($5 one-time fee)
- [ ] Get API credentials:
...
#### Integration Points
- [ ] Add notification to `LiveTradingEngine.on_trade_filled()`
  - Priority: 1 (High)
  - Title: "SPX Trade Filled"
  - Message: Include spread details, fill price, credit/debit
  
- [ ] Add notification to `LiveTradingEngine.on_position_closed()`
  - Priority: 1 (High)
  - Title: "Position Closed"
  - Message: Include P&L, hold time, exit reason
  
- [ ] Add notification to `LiveTradingEngine.on_stop_loss_hit()`
  - Priority: 2 (Emergency)
  - Title: "⚠️ STOP LOSS HIT"
  - Message: Include loss amount, strategy name
  
- [ ] Add notification to `WatcherService.on_service_down()`
  - Priority: 2 (Emergency)
  - Title: "🚨 SERVICE DOWN"
  - Message: Include service name, timestamp
  
- [ ] Add notification for daily drawdown limit
  - Priority: 2 (Emergency)
  - Title: "⚠️ DAILY DRAWDOWN LIMIT"
  - Message: Include current drawdown %, auto-disable status

#### Testing
- [ ] Test all notification types in paper trading
- [ ] Verify delivery time <5 seconds
- [ ] Test priority levels work correctly
- [ ] Test during market hours and after hours

**Code Example**:
```python
# In src/live_trading_service/engine.py
from quant_vibe.notifications.pushover import PushoverClient

class LiveTradingEngine:
    def __init__(self, ...):
        self.pushover = PushoverClient() if settings.PUSHOVER_ENABLED else None
    
    def on_trade_filled(self, trade):
        if self.pushover:
            self.pushover.send_alert(
                title="SPX Trade Filled",
                message=f"{trade.strategy_name}: {trade.spread_description} @ ${trade.fill_price:.2f}",
                priority=1
            )
```

**Success Criteria**: All critical events generate notifications within 5 seconds

---

### [ ] 3. Fix Bearish IV Scalp Strategy
**Priority**: HIGH  
**Estimated Time**: 4-8 hours (depends on approach)  
**Files**: `src/quant_vibe/strategies/bearish_iv_scalp.py`, `config/backtest.yaml`

**Context**: Strategy requires IV data, but historical data from Massive API has 0% IV coverage.

#### Option A: Disable for Backtesting (Quick - 15 minutes)
- [ ] Set `enabled: false` in `config/backtest.yaml`
- [ ] Document limitation in `docs/strategies/BEARISH_IV_SCALP_KNOWN_ISSUES.md`
- [ ] Enable ONLY for live trading where real-time IV is available
- [ ] Test live trading with paper mode first

#### Option B: Implement IV Calculation (Advanced - 6-8 hours)
- [ ] Create `src/quant_vibe/utils/black_scholes.py`
- [ ] Implement Black-Scholes IV solver using Newton-Raphson method
- [ ] Add risk-free rate data source (FRED API or hardcoded)
- [ ] Add SPX dividend yield (hardcode ~1.5% or fetch)
- [ ] Create backfill script: `scripts/backfill/backfill_historical_iv.py`
- [ ] Update `options_bars` table with calculated IV
- [ ] Validate calculated IV vs known values (spot-check)
- [ ] Re-run backtests to verify strategy generates trades

#### Option C: Use Alternative Signal (Intermediate - 4 hours)
- [ ] Create `src/quant_vibe/strategies/bearish_rv_scalp.py` (Realized Volatility)
...
- [ ] Compare results to IV-based approach (when available)

**Recommended**: Start with Option A, then implement Option C if you want bearish strategy for backtesting.

**Success Criteria**: 
- Option A: Strategy disabled in backtest, enabled in live with paper trading
- Option B: Historical backtests show >0 trades with reasonable results
- Option C: New strategy backtests successfully with similar risk/reward profile
...
**Files**: `src/quant_vibe/reporting/daily_report.py`, `scripts/generate_daily_report.py`

#### Implementation
- [ ] Create `src/quant_vibe/reporting/daily_report.py`:
```python
class DailyPerformanceReport:
    def __init__(self, target_daily_income: float = 1780.0):
        self.target = target_daily_income
    
    def generate_report(self, trades_df: pd.DataFrame, date: datetime) -> dict:
        """
        ...
        Returns:
            ...
            - actual_pnl: float
            - target_pnl: float
            - achievement_pct: float (actual/target)
            - win_rate: float
            - total_trades: int
            - by_strategy: dict (strategy-level metrics)
            - drawdown: float (max intraday drawdown)
            - slippage_avg: float (backtest vs actual fill difference)
        """
        pass
    
    def save_report(self, report: dict, output_dir: Path):
        """Save as JSON and CSV for historical tracking."""
        pass
    
    def send_summary(self, report: dict):
        """Send via Pushover at end of day (4:05 PM ET)."""
        pass
```

#### Features
- [ ] Actual P&L vs $1,780 target
- [ ] Achievement percentage (actual/target)
- [ ] Win rate by strategy
- [ ] Total trades executed
- [ ] Max intraday drawdown (vs 2% limit)
- [ ] Average slippage (backtest fill vs actual fill)
- [ ] Strategy-level breakdown
- [ ] Cumulative P&L for week/month

#### Automation
- [ ] Add cron job or scheduler to run at 4:05 PM ET daily
- [ ] Save reports to `reports/daily/YYYY-MM-DD_daily_report.json`
- [ ] Send Pushover notification with summary
- [ ] Generate weekly summary every Friday
- [ ] Generate monthly summary at month-end

#### Dashboard Integration
- [ ] Add daily report view to Admin UI
- [ ] Plot actual vs target on chart
- [ ] Show last 30 days of performance
- [ ] Highlight days below target in red

**Success Criteria**: Daily reports generated automatically, accessible via Admin UI and Pushover
