...
from .models import (
    ...
    OptimizationResult,
    ParamGridRequest,
    ParamGridValidationRequest,
    ParamGridResponse,
    ParamGridValidationResponse,
    FixedParamsResponse,
    QueueStatusResponse,
    CacheClearResponse,
)
...
async def get_config() -> OptimizationConfig:
    """Get or initialize configuration."""
    global _config
    if _config is None:
        _config = OptimizationConfig.from_environment()
    return _config


async def get_task_queue() -> OptimizationTaskQueue:
...
async def get_optimization_service() -> OptimizationService:
    ...
    if _optimization_service is None:
        ...
        logger.info(f"[API] DB connection: {config.db_user}@{config.db_host}:{config.db_port}/{config.db_name}")
...
async def get_param_grid(strategy_name: str):
    """
...
async def generate_param_grid(request: ParamGridRequest):
    """
...
async def validate_param_grid(request: ParamGridValidationRequest):
    """
...
async def get_fixed_params(strategy_name: str):
    """
...
async def run_optimization(request: OptimizationRequest):
    ...
    """
    try:
        logger.info(f"[API] Received optimization request: strategy={request.strategy_name}, "
                   f"dates={request.train_start_date} to {request.train_end_date}")
        service = await get_optimization_service()
