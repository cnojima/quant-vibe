...
class NaiveBullishPutStrategy(OptionsStrategy):
    ...
    def should_enter(
        ...
        if self.has_entered_today or not self.can_enter_new_position():
            return False

        # Enter at market open
        if not market_analysis.get('market_open', False):
            return False
        ...
        print(f"\n  🎯 NAIVE ENTRY SIGNAL - Market Open")
    ...
    def construct_spread(
        ...
        if short_put.empty or long_put.empty:
            print(f"  ⚠️  Cannot find both strikes: ${short_strike} and ${long_strike}")
            return None
        ...
        if (pd.isna(short_put_data['mark']) or
            ...
            print(f"  ⚠️  Invalid mark prices")
            return None

        print(f"\n  📋 Opening Spread:")
        print(f"     SELL {self.num_spreads}x {short_strike} PUT @ ${short_put_data['mark']:.2f}")
        print(f"     BUY  {self.num_spreads}x {long_strike} PUT @ ${long_put_data['mark']:.2f}")
        ...
        print(f"     Net Credit: ${net_credit:.2f} (per spread: ${net_credit_per_spread:.2f})")
