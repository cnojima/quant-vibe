...
interface LiveTradesChartProps {
...
export function LiveTradesChart({ data, underlyingData = [], trades = [] }: LiveTradesChartProps) {
      ...
      <div className="bg-white p-3 border border-gray-300 rounded shadow-lg">
        ...
        {tradeAtPoint && tradeAtPoint.trade && (
          <div className="mt-2 pt-2 border-t border-gray-200">
            ...
            {tradeAtPoint.trade.strategy && (
              <p className="text-xs text-gray-600">Strategy: {tradeAtPoint.trade.strategy}</p>
            )}
  ...
  const mergedData = data.map(equityPoint => {
    ...
    // Get the underlying price if within 30 minutes (live trading may have gaps)
    const currentBar = underlyingSorted[underlyingIndex];
    const timeDiff = equityTime - currentBar.time;
    const underlyingPrice = timeDiff >= 0 && timeDiff <= 30 * 60 * 1000
      ? currentBar.close
      : null;
        ...
        <div className="mt-4 p-4 bg-white border border-gray-300 rounded-lg shadow-md">
          <div className="flex justify-between items-start mb-3">
            <h3 className="text-lg font-semibold text-gray-800">
              Trade Details - {selectedTrade.strategy}
            </h3>
            <button
              onClick={() => setSelectedTradeIndex(null)}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <div className="mb-2">
                <span className="text-sm font-semibold text-gray-600">Position ID:</span>
                <p className="text-sm text-gray-800 font-mono">{selectedTrade.position_id}</p>
              </div>
              ...
              <div className="mb-2">
                <span className="text-sm font-semibold text-gray-600">Entry Cost:</span>
                <p className="text-base text-gray-800">{formatCurrency(selectedTrade.entry_cost)}</p>
              </div>

              <div className="mb-2">
                <span className="text-sm font-semibold text-gray-600">Exit Value:</span>
                <p className="text-base text-gray-800">{formatCurrency(selectedTrade.exit_value)}</p>
              </div>
            </div>
