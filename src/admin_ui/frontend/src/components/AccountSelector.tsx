...
export function AccountSelector({ tradingMode, selectedAccount, onAccountChange }: AccountSelectorProps) {
  ...
  function handleSetDefault(accountHash: string): void {
  ...
  function handleAccountSelect(accountHash: string): void {
    onAccountChange(accountHash);
    setIsOpen(false);
  }

  function showComingSoonToast(feature: string): void {
    addToast({
      type: 'info',
      title: 'Coming Soon',
  ...
  return (
    <div className="flex items-center space-x-2">
          ...
          <div className="absolute z-50 mt-2 w-80 bg-white border border-gray-200 rounded-lg shadow-lg">
            <div className="py-1">
              {accounts.map((account) => (
                <AccountOption
                  ...
                  account={account}
                  isSelected={account.account_hash === selectedAccount}
            ...
            <div className="border-t border-gray-200 px-4 py-2">
              <button
                onClick={() => {
                  setIsOpen(false);
                  showComingSoonToast('Account management features');
                }}
      ...
      <button
        onClick={() => showComingSoonToast(`Order synchronization for ${selectedAccountData?.nickname || 'this account'}`)}
        className="p-2 hover:bg-gray-100 rounded-lg transition-colors"
...
interface AccountOptionProps {
  ...
  onSetDefault: (accountHash: string) => void;
}

function AccountOption({ account, isSelected, onSelect, onSetDefault }: AccountOptionProps) {
  return (
    <button
      onClick={() => onSelect(account.account_hash)}
      ...
      <div className="flex items-start justify-between">
        ...
        {!account.is_default && (
          <button
            onClick={(e) => {
              e.stopPropagation();
              onSetDefault(account.account_hash);
            }}
