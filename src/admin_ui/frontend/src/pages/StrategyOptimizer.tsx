...
import { ParamGridEditor } from '../components/forms/ParamGridEditor';
import { FixedParamsPanel } from '../components/forms/FixedParamsPanel';
...
export function StrategyOptimizer() {
  ...
  const [customParamGrid, setCustomParamGrid] = useState<Record<string, any[]> | null>(null);
  const [customFixedParams, setCustomFixedParams] = useState<Record<string, any> | null>(null);
  ...
  const { data: autoParamGrid } = useParamGrid(selectedStrategy);
  const { data: fixedParamsData } = useFixedParams(selectedStrategy);
  const cancelOptimization = useCancelOptimization();

  // Load auto-generated param grid when strategy changes
  useEffect(() => {
    if (autoParamGrid && !customParamGrid) {
      setCustomParamGrid(autoParamGrid.param_grid);
    }
  }, [autoParamGrid, selectedStrategy]);

  // Reset param grid when strategy changes
  useEffect(() => {
    setCustomParamGrid(null);
    setCustomFixedParams(null);
  }, [selectedStrategy]);
  ...
  const handleRunOptimization = async () => {
    ...
    try {
      const result = await runOptimization.mutateAsync({
        ...
        param_grid: customParamGrid || undefined,
        fixed_params: customFixedParams || undefined,
      });
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card>
            ...
            <div className="space-y-4">
              ...
              {selectedStrategy && customParamGrid && (
                <div className="pt-4 border-t border-gray-200">
                  <ParamGridEditor
                    strategyName={selectedStrategy}
                    initialGrid={customParamGrid}
                    onChange={(grid) => setCustomParamGrid(grid)}
                    maxCombinations={200}
                  />
                </div>
              )}
              ...
              {selectedStrategy && fixedParamsData && (
                <div className="pt-4">
                  <FixedParamsPanel
                    fixedParams={customFixedParams || fixedParamsData.fixed_params}
                    onChange={(params) => setCustomFixedParams(params)}
                    editable={false}
                  />
                </div>
              )}
                ...
                <div className="flex items-center gap-4">
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      {optimizationStatus.status === 'running' && currentOptimizationId && (
                        <Button
                          variant="secondary"
                          size="sm"
                          onClick={async () => {
                            if (confirm('Are you sure you want to cancel this optimization?')) {
                              try {
                                await cancelOptimization.mutateAsync(currentOptimizationId);
                                alert('Optimization cancelled');
                              } catch (error) {
                                console.error('Failed to cancel optimization:', error);
                                alert('Failed to cancel optimization. Please try again.');
                              }
                            }
                          }}
                        ...
                        >
                          {cancelOptimization.isPending ? 'Cancelling...' : 'Cancel Optimization'}
                        </Button>
                      )}
                    </div>
                    ...
                    {optimizationStatus.current_combination !== undefined && optimizationStatus.total_combinations !== undefined && (
                      <div className="mt-2">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          {optimizationStatus.estimated_completion_time && (
                            <div className="text-sm text-gray-600">
                              ETA: {formatDistanceToNow(new Date(optimizationStatus.estimated_completion_time), { addSuffix: true })}
                            </div>
                          )}
                        </div>
                          ...
                          <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded">
                            ...
                            {optimizationStatus.current_metrics && (
                              <div className="mt-2 grid grid-cols-2 md:grid-cols-4 gap-2 text-xs pt-2 border-t border-blue-300">
                                ...
                                {optimizationStatus.current_metrics.total_trades !== undefined && (
                                  <div>
                                    <span className="text-blue-700">Trades:</span>{' '}
                                    <span className="font-semibold text-blue-900">
                                      {optimizationStatus.current_metrics.total_trades}
                                    </span>
                                  </div>
                                )}
                              </div>
